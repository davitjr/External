...
{
    internal class BillSplitReminderOrderDB
    {
        internal static ActionResult Save(BillSplitReminderOrder order, string userName, SourceType source)
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HbBaseConn"].ToString()))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    conn.Open();
                    cmd.Connection = conn;
                    cmd.CommandText = "pr_submit_bill_split_reminder_order";
                    ...
                    cmd.Parameters.Add("@descr_for_payment", SqlDbType.NVarChar).Value = order.SenderId.ToString();
        ...
        internal static BillSplitReminderOrder GetBillSplitReminderOrder(BillSplitReminderOrder order)
        {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
                        {
                            ...
                            if (dr["descr_for_payment"] != DBNull.Value)
                                order.SenderId = Convert.ToInt32(dr["descr_for_payment"].ToString());
        ...
        internal static int GetBillSplitSenderReminderOrdersCount(int senderId)
        {
            int ordersCount = 0;
            ...
            using (SqlConnection conn = new SqlConnection(WebConfigurationManager.ConnectionStrings["HBBaseConn"].ToString()))
            {
                ...
                {
                    cmd.Parameters.Add("@senderId", SqlDbType.Float).Value = senderId.ToString();
