...
{
    static class CreditLineCardReplaceOrderDB
    {
        /// <summary>
        /// Փոխարինում` նոր համար, նոր ժամկետ - վ․գ․ հայտի տվյալների պահպանում
        /// </summary>
        /// <param name="order"></param>
        /// <param name="user"></param>
        /// <param name="source"></param>
        /// <returns></returns>
        internal static ActionResult Save(CreditLineCardReplaceOrder order, SourceType source, ACBAServiceReference.User user)
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HBBaseConn"].ToString()))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    conn.Open();
                    cmd.Connection = conn;
                    cmd.CommandText = "pr_credit_line_card_replace_order";
        ...
        /// <param name="ID"></param>
        /// <returns></returns>
        internal static CreditLineCardReplaceOrder GetCreditLineCardReplaceOrder(CreditLineCardReplaceOrder order)
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HbBaseConn"].ToString()))
            {
                ...
                SqlCommand cmd = new SqlCommand(@" SELECT D.app_id AS app_id,
                                                    ...
                                                    FROM Tbl_HB_documents H 
                                                         LEFT JOIN tbl_credit_Line_card_replace_order_details D 
        ...
        internal static bool CheckCustomerDDocument(ulong customerNumber)
        {
        ...
        internal static ulong GetCardNewAppID(CreditLineCardReplaceOrder order)
        {
            ulong newAppID = 0;
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["AccOperBaseConn"].ToString()))
            {
        ...
        internal static bool IsAlreadyExistReplaceOrder(long productId)
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["AccOperBaseConn"].ToString()))
            {
                string sql = @"SELECT 1 FROM tbl_hb_documents H
                                    INNER JOIN tbl_credit_line_card_replace_order_details D
