...
namespace ExternalBanking.DBManager
{
    public class DepositaryAccountOrderDB
    {
        ...
        {
            DepositaryAccount depositaryAccountOrder = new DepositaryAccount();
        ...
        internal static DepositaryAccountOrder GetDepositaryAccountOrder(DepositaryAccountOrder order)
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HbBaseConn"].ToString()))
            {
                using (SqlCommand cmd = new SqlCommand(sql, conn))
                {
                    ...
                    order.AccountNumber.AccountNumber = dt.Rows[0]["depositary_account"] != DBNull.Value ? Convert.ToDouble(dt.Rows[0]["depositary_account"].ToString()) : 0;
                    order.AccountNumber.BankCode = dt.Rows[0]["depositary_account_bank_code"] != DBNull.Value ? Convert.ToInt32(dt.Rows[0]["depositary_account_bank_code"]) : 0;
                    order.AccountNumber.Description = dt.Rows[0]["depositary_account_description"] != DBNull.Value ? Utility.ConvertAnsiToUnicode(dt.Rows[0]["depositary_account_description"].ToString()) : "";
                }
            }
            return order;
        }
