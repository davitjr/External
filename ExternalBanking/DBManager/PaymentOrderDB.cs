...
namespace ExternalBanking.DBManager
{
    internal static class PaymentOrderDB
    {
                    ...
                    using (SqlDataReader dr = cmd.ExecuteReader())
                    {
                        if (dr.Read())
                        {
                            ...
                            if (order.Type == OrderType.InterBankTransferCash || order.Type == OrderType.InterBankTransferNonCash)
                            {
                                ...
                                order.AdditionalParametrs.Add(new AdditionalDetails
                                {
        ...
        internal static ActionResult Save(PaymentOrder order, string userName, SourceType source)
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HbBaseConn"].ToString()))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    if (order?.DebitAccount?.IsAttachedCard == true && order.Type == OrderType.RATransfer && order.ReceiverBankCode == 0) // Կցված քարտով վարկի մարման համար (Կցված քարտից փոխանցում վարկային կոդին)
                    {
                    ...
                    if ((order.Source == SourceType.AcbaOnline || order.Source == SourceType.MobileBanking) && order.Type == OrderType.RATransfer && order.SubType == 1)
                    {
                        if (order.AdditionalParametrs != null && order.AdditionalParametrs.Exists(m => m.AdditionTypeDescription == "BillSplitSenderId"))
        ...
        internal static ActionResult SaveCash(PaymentOrder order, string userName, SourceType source)
        {
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HbBaseConn"].ToString()))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("pr_save_payment_order_cash", conn))
                {
                    ...
                    cmd.Parameters.Add("@receiver_name", SqlDbType.NVarChar, 250).Value = order.Receiver == null ? "" : order.Receiver;
                    cmd.Parameters.Add("@credit_bank_code", SqlDbType.VarChar, 5).Value = order.ReceiverBankCode;
                    if (order.ValidateForConvertation)
                    {
                        cmd.Parameters.Add("@cust_rate", SqlDbType.Float).Value = order.ConvertationRate;
        ...
        internal static Tuple<bool, string> IsBigAmount(string type, string accountNumber, double amount, string currency, ulong customerNumber, DateTime operationDate)
        {
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["AccOperBaseConn"].ToString()))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    cmd.ExecuteNonQuery();
                    bool isBigAmount = (Convert.ToInt32(cmd.Parameters["@isBigAmount"].Value) == 1) ? true : false;
            ...
            using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HbBaseConn"].ToString()))
            {
                ...
                using (SqlCommand cmd = new SqlCommand(query, conn))
                {
                    ...
                    for (int i = 0; i < dt.Rows.Count; i++)
                    {
                        ...
                        reestrTransferAdditionalDetails.Amount = Convert.ToDouble(row["amount"].ToString());
            ...
            if (reestrTransferAdditionalDetails != null)
            {
                using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HbBaseConn"].ToString()))
                {
                    using (SqlCommand cmd = new SqlCommand())
                    {
                        ...
                        {
                            rowsN_invalidCred = dataTable.Rows[0]["rowsEnum"].ToString();
                            rowsN_sameAcc = dataTable.Rows[1]["rowsEnum"].ToString();
                            rowsN = dataTable.Rows[2]["rowsEnum"].ToString();
                            rowsN_unicode = dataTable.Rows[3]["rowsEnum"].ToString();
                            rowsN_cent = dataTable.Rows[4]["rowsEnum"].ToString();
                            rowsN_Buge_Transfer = dataTable.Rows[5]["rowsEnum"].ToString();
                            rowsN_Bank_Closed = dataTable.Rows[6]["rowsEnum"].ToString();
        ...
        internal static ActionResult SaveReestr(ReestrTransferOrder order, string userName, SourceType source, string fileId, Action action, Languages languages = Languages.hy)
        {
            ...
            //cmd.Parameters.Add(new SqlParameter("@msg", SqlDbType.NVarChar, 4000)
            //{ Direction = ParameterDirection.Output });
            ...
            //cmd.Parameters.Add(new SqlParameter("@err_id", SqlDbType.Int)
            //{ Direction = ParameterDirection.Output });


            //cmd.Parameters.Add("@dt", SqlDbType.Structured).Value = ReestrTransferAdditionalDetails.ConvertAdditionalReestrDetailsToDataTable(reestrTransferAdditionalDetails, languages);
