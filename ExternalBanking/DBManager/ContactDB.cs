	...
	internal class ContactDB
	{
		internal static void Add(Contact contact, ulong customerNumber, SqlConnection conn)
		{
			...
			if (isTestVersion)
			{
				...
				{
					cmd.Connection = conn;
					cmd.CommandText = @"INSERT INTO tbl_Contacts (Description,customerNumber,PhoneNumber,Email,Picture,IsNew) Values(@Description,@customerNumber,@phoneNumber,@email,@picture,1) Select SCOPE_IDENTITY() as ID ";
		...
		internal static void Update(Contact contact, SqlConnection connection)
		{
			...
			if (isTestVersion)
			{
				using (SqlCommand command = new SqlCommand())
				{
					...
					command.CommandText = @"UPDATE tbl_Contacts SET Description = @Description, email = @email, phonenumber = @phonenumber, Picture = @picture, isnew = 1 WHERE ID =@ID ";
		...
		internal static Contact GetContact(ulong contactId)
		{
			...
			using (SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HBBaseConn"].ToString()))
			{
				...
				if (isTestVersion)
				{
					...
					{
						cmd.Connection = conn;
						cmd.CommandText = @"SELECT ID,Description,phonenumber,email,picture FROM tbl_Contacts WHERE ID =@ID ";
		...
		internal static List<Contact> GetContacts(ulong customerNumber, SqlConnection connection)
		{
			...
			if (isTestVersion)
			{
				using (SqlCommand cmd = new SqlCommand())
				{
					...
					cmd.CommandText = @"SELECT ID,Description,phonenumber,email,picture FROM tbl_Contacts WHERE customerNumber =@customerNumber ";
					cmd.Parameters.Add("@customerNumber", SqlDbType.Float).Value = customerNumber;
