    ...
    static class AccountDataChangeOrderDB
    {
        ...
        internal static bool IsSecondDataChange(ulong customerNumber, string accountNumber)
        {
            ...
            using SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["HbBaseConn"].ToString());
                ...
                conn.Open();
               using SqlCommand cmd = new SqlCommand(@"Select doc_ID from Tbl_HB_documents where quality in (1,2,3,5) and document_type=50 and document_subtype=1 and
        ...
        internal static bool IsSameAdditionalDataChange(string accountNumber, AdditionalDetails additionalDetails)
        {
            using SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["AccOperBaseConnRO"].ToString());
                ...
                conn.Open();
               using SqlCommand cmd = new SqlCommand(@"SELECT 1 FROM Tbl_all_accounts_AddInf 
        ...
        public static AdditionalValueType GetAccountAdditionValueType(ushort additionType)
        {
            ...
            using SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["AccOperBaseConnRO"].ToString());
                ...
                conn.Open();
              using SqlCommand cmd = new SqlCommand("SELECT DataType FROM Tbl_type_of_all_acc_additions WHERE AdditionId = @additionId", conn);
        ...
        internal static bool IsApprovedByTaxService(string accountNumber)
        {
            using SqlConnection conn = new SqlConnection(ConfigurationManager.ConnectionStrings["AccOperBaseConnRO"].ToString());
            conn.Open();
            using SqlCommand cmd = new SqlCommand(@"SELECT account_number FROM Tbl_acc_freeze_history WHERE reason_type=14 AND closing_date IS NULL AND account_number=@accountNumber", conn);
            cmd.Parameters.Add("@accountNumber", SqlDbType.Float).Value = accountNumber;
